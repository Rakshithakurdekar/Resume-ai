...
# ---------------------------------
# DOCX Generation Function (remains the same)
# ---------------------------------
...
def load_images():
    try:
        hero = Image.open("image.png")
        resume = Image.open("image copy 2.png")
    except:
        hero = Image.new('RGB', (8, 5), color='#3498db')
        resume = Image.new('RGB', (100, 100), color='#2ecc71')
        st.warning("⚠️ Using placeholder images. Missing image files.")
    return hero, resume

HERO_IMAGE, RESUME_ICON = load_images()
...
def landing_page():
    ...
    # Resize hero image to exact size
    resized_hero = HERO_IMAGE.resize((700, 500))  # width=700, height=500

    # Center the image
    st.markdown(
        f"""
        <div style="text-align:center;">
            <img src="data:image/png;base64,{image_to_base64(resized_hero)}" 
    ...
    for col, feature in zip(cols, features):
        with col:
            # UPDATED: Reduced feature icon width to 80px
            st.image(feature[2], width=80) 
            st.markdown(f"**{feature[0]}**")
            st.markdown(feature[1])

    st.markdown("---")

    # --- About/How it Works Section (remains the same) ---
...
# ======================================================
#                  RESUME MATCHER PAGE (remains the same)
# ======================================================
